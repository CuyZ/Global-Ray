 ...
 */
class NamedParameterMap extends \RayGlobalScoped\Ramsey\Collection\Map\AbstractMap
{
    use TypeTrait;
    ...
    public function offsetSet($offset, $value) : void
    {
        ...
        if (!\array_key_exists($offset, $this->namedParameters)) {
            throw new \RayGlobalScoped\Ramsey\Collection\Exception\InvalidArgumentException('Attempting to set value for unconfigured parameter \'' . $offset . '\'');
        }
        if ($this->checkType($this->namedParameters[$offset], $value) === \false) {
            throw new \RayGlobalScoped\Ramsey\Collection\Exception\InvalidArgumentException('Value for \'' . $offset . '\' must be of type ' . $this->namedParameters[$offset] . '; value is ' . $this->toolValueToString($value));
        }
