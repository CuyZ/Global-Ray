 ...
 * @psalm-immutable
 */
class PhpTimeConverter implements \RayGlobalScoped\Ramsey\Uuid\Converter\TimeConverterInterface
{
    /**
    ...
    public function __construct(?\RayGlobalScoped\Ramsey\Uuid\Math\CalculatorInterface $calculator = null, ?\RayGlobalScoped\Ramsey\Uuid\Converter\TimeConverterInterface $fallbackConverter = null)
    {
        if ($calculator === null) {
            $calculator = new \RayGlobalScoped\Ramsey\Uuid\Math\BrickMathCalculator();
        }
        if ($fallbackConverter === null) {
            $fallbackConverter = new \RayGlobalScoped\Ramsey\Uuid\Converter\Time\GenericTimeConverter($calculator);
        }
    ...
    {
        $seconds = new \RayGlobalScoped\Ramsey\Uuid\Type\Integer($seconds);
        $microseconds = new \RayGlobalScoped\Ramsey\Uuid\Type\Integer($microseconds);
        ...
        return new \RayGlobalScoped\Ramsey\Uuid\Type\Hexadecimal(\str_pad(\dechex((int) $uuidTime), 16, '0', \STR_PAD_LEFT));
    }
    public function convertTime(\RayGlobalScoped\Ramsey\Uuid\Type\Hexadecimal $uuidTimestamp) : \RayGlobalScoped\Ramsey\Uuid\Type\Time
    {
        ...
        return new \RayGlobalScoped\Ramsey\Uuid\Type\Time($splitTime['sec'], $splitTime['usec']);
    }
    /**
