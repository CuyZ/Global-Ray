 ...
 */
class CombGenerator implements \RayGlobalScoped\Ramsey\Uuid\Generator\RandomGeneratorInterface
{
     ...
     */
    private $converter;
    public function __construct(\RayGlobalScoped\Ramsey\Uuid\Generator\RandomGeneratorInterface $generator, \RayGlobalScoped\Ramsey\Uuid\Converter\NumberConverterInterface $numberConverter)
    {
    ...
    public function generate(int $length) : string
    {
        if ($length < self::TIMESTAMP_BYTES || $length < 0) {
            throw new \RayGlobalScoped\Ramsey\Uuid\Exception\InvalidArgumentException('Length must be a positive integer greater than or equal to ' . self::TIMESTAMP_BYTES);
        }
        $hash = '';
