 ...
 * @psalm-immutable
 */
class OrderedTimeCodec extends \RayGlobalScoped\Ramsey\Uuid\Codec\StringCodec
{
    /**
    ...
    {
        if (!$uuid->getFields() instanceof \RayGlobalScoped\Ramsey\Uuid\Rfc4122\FieldsInterface || $uuid->getFields()->getVersion() !== \RayGlobalScoped\Ramsey\Uuid\Uuid::UUID_TYPE_TIME) {
            throw new \RayGlobalScoped\Ramsey\Uuid\Exception\InvalidArgumentException('Expected RFC 4122 version 1 (time-based) UUID');
        }
    ...
    public function decodeBytes(string $bytes) : \RayGlobalScoped\Ramsey\Uuid\UuidInterface
    {
        ...
        if (!$uuid->getFields() instanceof \RayGlobalScoped\Ramsey\Uuid\Rfc4122\FieldsInterface || $uuid->getFields()->getVersion() !== \RayGlobalScoped\Ramsey\Uuid\Uuid::UUID_TYPE_TIME) {
            throw new \RayGlobalScoped\Ramsey\Uuid\Exception\UnsupportedOperationException('Attempting to decode a non-time-based UUID using ' . 'OrderedTimeCodec');
        }
        return $uuid;
    }
}
