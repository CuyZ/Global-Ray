...
final class Fields implements \RayGlobalScoped\Ramsey\Uuid\Rfc4122\FieldsInterface
{
    ...
    public function __construct(string $bytes)
    {
        ...
        if (!$this->isCorrectVariant()) {
            throw new \RayGlobalScoped\Ramsey\Uuid\Exception\InvalidArgumentException('The byte string received does not conform to the RFC 4122 variant');
        }
        if (!$this->isCorrectVersion()) {
            throw new \RayGlobalScoped\Ramsey\Uuid\Exception\InvalidArgumentException('The byte string received does not contain a valid RFC 4122 version');
        }
    }
    ...
    public function getTimestamp() : \RayGlobalScoped\Ramsey\Uuid\Type\Hexadecimal
    {
        switch ($this->getVersion()) {
            case \RayGlobalScoped\Ramsey\Uuid\Uuid::UUID_TYPE_DCE_SECURITY:
                $timestamp = \sprintf('%03x%04s%08s', \hexdec($this->getTimeHiAndVersion()->toString()) & 0xfff, $this->getTimeMid()->toString(), '');
                break;
            case \RayGlobalScoped\Ramsey\Uuid\Uuid::UUID_TYPE_PEABODY:
        ...
        return new \RayGlobalScoped\Ramsey\Uuid\Type\Hexadecimal($timestamp);
    }
    ...
    private function isCorrectVariant() : bool
    {
        ...
        return $this->getVariant() === \RayGlobalScoped\Ramsey\Uuid\Uuid::RFC_4122;
    }
}
