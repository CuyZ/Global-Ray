...
use RayGlobalScoped\Ramsey\Uuid\Exception\InvalidBytesException;
...
trait VariantTrait
{
    ...
    public function getVariant() : int
    {
        if (\strlen($this->getBytes()) !== 16) {
            throw new \RayGlobalScoped\Ramsey\Uuid\Exception\InvalidBytesException('Invalid number of bytes');
        }
        ...
        if ($msb === '111') {
            $variant = \RayGlobalScoped\Ramsey\Uuid\Uuid::RESERVED_FUTURE;
        } elseif ($msb === '110') {
            $variant = \RayGlobalScoped\Ramsey\Uuid\Uuid::RESERVED_MICROSOFT;
        } elseif (\strpos($msb, '10') === 0) {
            $variant = \RayGlobalScoped\Ramsey\Uuid\Uuid::RFC_4122;
        } else {
            $variant = \RayGlobalScoped\Ramsey\Uuid\Uuid::RESERVED_NCS;
        }
        return $variant;
    }
}
