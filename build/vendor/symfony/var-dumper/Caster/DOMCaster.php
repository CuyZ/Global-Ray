...
class DOMCaster
{
    ...
    public static function castDocument(\DOMDocument $dom, array $a, \RayGlobalScoped\Symfony\Component\VarDumper\Cloner\Stub $stub, bool $isNested, int $filter = 0)
    {
        $a += ['doctype' => $dom->doctype, 'implementation' => $dom->implementation, 'documentElement' => new \RayGlobalScoped\Symfony\Component\VarDumper\Caster\CutStub($dom->documentElement), 'actualEncoding' => $dom->actualEncoding, 'encoding' => $dom->encoding, 'xmlEncoding' => $dom->xmlEncoding, 'standalone' => $dom->standalone, 'xmlStandalone' => $dom->xmlStandalone, 'version' => $dom->version, 'xmlVersion' => $dom->xmlVersion, 'strictErrorChecking' => $dom->strictErrorChecking, 'documentURI' => $dom->documentURI ? new \RayGlobalScoped\Symfony\Component\VarDumper\Caster\LinkStub($dom->documentURI) : $dom->documentURI, 'config' => $dom->config, 'formatOutput' => $dom->formatOutput, 'validateOnParse' => $dom->validateOnParse, 'resolveExternals' => $dom->resolveExternals, 'preserveWhiteSpace' => $dom->preserveWhiteSpace, 'recover' => $dom->recover, 'substituteEntities' => $dom->substituteEntities];
        if (!($filter & \RayGlobalScoped\Symfony\Component\VarDumper\Caster\Caster::EXCLUDE_VERBOSE)) {
            $formatOutput = $dom->formatOutput;
            $dom->formatOutput = \true;
            $a += [\RayGlobalScoped\Symfony\Component\VarDumper\Caster\Caster::PREFIX_VIRTUAL . 'xml' => $dom->saveXML()];
            $dom->formatOutput = $formatOutput;
        }
        return $a;
    }
    public static function castCharacterData(\DOMCharacterData $dom, array $a, \RayGlobalScoped\Symfony\Component\VarDumper\Cloner\Stub $stub, bool $isNested)
    {
        $a += ['data' => $dom->data, 'length' => $dom->length];
        return $a;
    }
    public static function castAttr(\DOMAttr $dom, array $a, \RayGlobalScoped\Symfony\Component\VarDumper\Cloner\Stub $stub, bool $isNested)
    {
        $a += ['name' => $dom->name, 'specified' => $dom->specified, 'value' => $dom->value, 'ownerElement' => $dom->ownerElement, 'schemaTypeInfo' => $dom->schemaTypeInfo];
        return $a;
    }
    public static function castElement(\DOMElement $dom, array $a, \RayGlobalScoped\Symfony\Component\VarDumper\Cloner\Stub $stub, bool $isNested)
    {
        $a += ['tagName' => $dom->tagName, 'schemaTypeInfo' => $dom->schemaTypeInfo];
        return $a;
    }
    public static function castText(\DOMText $dom, array $a, \RayGlobalScoped\Symfony\Component\VarDumper\Cloner\Stub $stub, bool $isNested)
    {
        $a += ['wholeText' => $dom->wholeText];
        return $a;
    }
    public static function castTypeinfo(\DOMTypeinfo $dom, array $a, \RayGlobalScoped\Symfony\Component\VarDumper\Cloner\Stub $stub, bool $isNested)
    {
        $a += ['typeName' => $dom->typeName, 'typeNamespace' => $dom->typeNamespace];
        return $a;
    }
    public static function castDomError(\DOMDomError $dom, array $a, \RayGlobalScoped\Symfony\Component\VarDumper\Cloner\Stub $stub, bool $isNested)
    {
    ...
    public static function castLocator(\DOMLocator $dom, array $a, \RayGlobalScoped\Symfony\Component\VarDumper\Cloner\Stub $stub, bool $isNested)
    {
        $a += ['lineNumber' => $dom->lineNumber, 'columnNumber' => $dom->columnNumber, 'offset' => $dom->offset, 'relatedNode' => $dom->relatedNode, 'uri' => $dom->uri ? new \RayGlobalScoped\Symfony\Component\VarDumper\Caster\LinkStub($dom->uri, $dom->lineNumber) : $dom->uri];
        return $a;
    }
    public static function castDocumentType(\DOMDocumentType $dom, array $a, \RayGlobalScoped\Symfony\Component\VarDumper\Cloner\Stub $stub, bool $isNested)
    {
    ...
    public static function castNotation(\DOMNotation $dom, array $a, \RayGlobalScoped\Symfony\Component\VarDumper\Cloner\Stub $stub, bool $isNested)
    {
        $a += ['publicId' => $dom->publicId, 'systemId' => $dom->systemId];
        return $a;
    }
    public static function castEntity(\DOMEntity $dom, array $a, \RayGlobalScoped\Symfony\Component\VarDumper\Cloner\Stub $stub, bool $isNested)
    {
    ...
    public static function castProcessingInstruction(\DOMProcessingInstruction $dom, array $a, \RayGlobalScoped\Symfony\Component\VarDumper\Cloner\Stub $stub, bool $isNested)
    {
        $a += ['target' => $dom->target, 'data' => $dom->data];
        return $a;
    }
    public static function castXPath(\DOMXPath $dom, array $a, \RayGlobalScoped\Symfony\Component\VarDumper\Cloner\Stub $stub, bool $isNested)
    {
