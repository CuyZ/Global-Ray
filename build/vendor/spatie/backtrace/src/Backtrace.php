...
namespace RayGlobalScoped\Spatie\Backtrace;
...
class Backtrace
{
    ...
    protected function toFrameObjects(array $rawFrames) : array
    {
        ...
        foreach ($rawFrames as $rawFrame) {
            $frames[] = new \RayGlobalScoped\Spatie\Backtrace\Frame($currentFile, $currentLine, $rawFrame['args'] ?? null, $rawFrame['function'] ?? null, $rawFrame['class'] ?? null, $this->isApplicationFrame($currentFile));
            $currentFile = $rawFrame['file'] ?? 'unknown';
            $currentLine = $rawFrame['line'] ?? 0;
        }
        $frames[] = new \RayGlobalScoped\Spatie\Backtrace\Frame($currentFile, $currentLine, [], '[top]');
    ...
    {
        return $this->startAtFrameFromClosure($frames, function (\RayGlobalScoped\Spatie\Backtrace\Frame $frame) {
