...
use Exception;
use RayGlobalScoped\Spatie\Ray\Exceptions\StopExecutionRequested;
class Client
{
    ...
    public function send(\RayGlobalScoped\Spatie\Ray\Request $request) : void
    {
    ...
    public function lockExists(string $lockName) : bool
    {
        ...
        try {
            ...
            if ($response['stop_execution'] ?? \false) {
                throw \RayGlobalScoped\Spatie\Ray\Exceptions\StopExecutionRequested::make();
            }
            return $response['active'] ?? \false;
        } catch (\Exception $exception) {
            if ($exception instanceof \RayGlobalScoped\Spatie\Ray\Exceptions\StopExecutionRequested) {
                throw $exception;
            }
        } finally {
            \curl_close($curlHandle);
